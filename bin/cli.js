...
program
    ...
    .action(function(file, outputfile, options) {
      ...
      const result = JSON.stringify(parser.parse(content));
      if (outputfile) {
        const outputFile = path.join(process.cwd(), outputfile);
...
program
    ...
    .action(function(dir, options) {
       ...
       * @type {[type]}
       */
      const directoryArray = fs.readdirSync(dirPath);
      const outputPath = dirPath + '/tldr.json';
      ...
      directoryArray.map((item) => {
        ...
        if (info.isFile() && path.extname(item) == '.md') {
          ...
          const tldrObj = parser.parse(content);
